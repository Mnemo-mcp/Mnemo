    ...
    <section className="py-24 px-6">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-bold text-center mb-3">
          <span className="text-accent">Built in layers</span>
        </h2>
        <p className="text-gray-400 text-sm text-center mb-12 max-w-xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {phases.map(p => (
            <div key={p.phase} className={`card p-5 border-t-2 ${p.color}`}>
              <div className="text-[10px] text-gray-500 mb-1 font-semibold">{p.phase}</div>
              <h3 className="font-semibold text-sm mb-3">{p.title}</h3>
              <ul className="space-y-1">
                {p.items.map(item => (
                  <li key={item} className="text-[11px] text-gray-400 flex items-start gap-2">
