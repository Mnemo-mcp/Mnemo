  ...
  return (
    <section className="py-28 px-6">
      <div className="max-w-5xl mx-auto">
        <div className="mb-16">
          <span className="text-xs font-semibold tracking-widest text-accent uppercase">Compatibility</span>
        ...
        <div className="grid sm:grid-cols-2 gap-3 mb-8">
          {FIRST_PARTY.map(a => (
            <div key={a.name} className="card p-5 hover:border-accent/30 transition-colors">
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-semibold text-white text-sm">{a.name}</h3>
                <span className="text-[10px] font-semibold tracking-wider px-2 py-0.5 rounded-full bg-accent-subtle text-accent border border-accent/30">{a.tag}</span>
              </div>
        ...
        <div className="flex flex-wrap gap-2">
          {COMMUNITY.map(name => (
            <div key={name} className="px-3 py-2 bg-surface-raised border border-border-subtle rounded-lg text-xs text-gray-400 hover:text-white hover:border-gray-500 transition-all">
              {name}
            </div>
          ))}
        </div>
